...
contract GSWSafeV1 {
    ...
    constructor(address[] memory _owners, uint256 _threshold, address _executor) {
        uint256 len = _owners.length;
        if (len < _threshold || _threshold == 0) revert InvalidThreshold();
    ...
    function execute(
        ...
        // consume the nonce only after signatures validated
    ...
    function setExecutor(address _executor, bytes calldata signatures) external {
        if (_executor == address(0)) revert ZeroAddress();

        uint256 currentNonce = nonce;
        bytes32 structHash = keccak256(abi.encode(
            ADMIN_TYPEHASH, keccak256(bytes("setExecutor")), _executor, uint256(0), currentNonce
        ));
        bytes32 hash = _hashTypedData(structHash);
        ...
        emit ExecutorChanged(executor, _executor);
        executor = _executor;
    }

    function addOwner(address owner, uint256 newThreshold, bytes calldata signatures) external {
        if (owner == address(0)) revert InvalidOwner();
        if (isOwner[owner]) revert DuplicateOwner();
    ...
    function removeOwner(address owner, uint256 newThreshold, bytes calldata signatures) external {
        ...
        // remove owner (swap & pop)
        uint256 idx = ownerIndex[owner];
        if (idx == 0) revert NotOwner(); // defensive
    ...
    function setThreshold(uint256 newThreshold, bytes calldata signatures) external {
    ...
    function cancelNonce(bytes calldata signatures) external nonReentrant {
        ...
        // consume the nonce and emit the consumed nonce
        nonce = currentNonce + 1;
    ...
    function _recover(bytes32 hash, bytes calldata sig) internal pure returns (address signer) {
        ...
        uint8 v;
        assembly {
            // sig.offset is the start position in calldata
            r := calldataload(sig.offset)
            s := calldataload(add(sig.offset, 32))
            // load the word and extract first byte
    ...
    /// @notice Returns the hash signers should sign for a transaction using the current nonce.
    ...
    /// @notice Returns the hash signers should sign for an admin action using the current nonce.
    ...
    /// @notice Returns the full list of owners (useful for frontends).
